...
`seqfu msa` is the preferred command. The legacy binary `fu-msa` is still shipped for existing workflows: it prints a short migration notice, waits three seconds, and then runs `seqfu msa` with the same arguments.
...
For compatibility, `fu-msa` still accepts the same arguments:

```bash
fu-msa {input_file} --setting-string Seq:0:6:20
```

It forwards to `seqfu msa` after printing a short migration message.

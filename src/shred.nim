...
proc shredMain(args: var seq[string], cmdName = "shred"): int =
  ...
  if verbose:
    stderr.writeLine("[", cmdName, "] Read length: ", readLength)
    stderr.writeLine("[", cmdName, "] Step: ", step, if doRevComp: " (with reverse complement)" else: "")
    if pe:
      stderr.writeLine("[", cmdName, "] Fragment length: ", fragLength)
      stderr.writeLine("[", cmdName, "] Output prefix: ", outprefix)
  ...
  for inputFile in inputFiles:
    ...
    if verbose:
      stderr.writeLine("[", cmdName, "] Processing file: ", inputFile, if pe: " (paired end)" else: "(single end)")
    try:

...
proc drawSeqs(MSA: msa, coords: coordinates) =
  let
    ...
    termWidth = max(terminalWidth(), coords.labelwidth + 20)
    termHeight = max(terminalHeight(), 12)
...
proc help() =
  var tb = newTerminalBuffer(max(terminalWidth(), 40), max(terminalHeight(), 18))
...
proc msaMain(argv: var seq[string], cmdName = "msa"): int =
  let args = docopt("""
  Usage:
    $CMD$ [options] <MSAFILE>
  ...
  """.replace("$CMD$", cmdName), version="1.0", argv=argv)
...
proc seqfuMsa*(argv: var seq[string]): int {.gcsafe.} =
  {.cast(gcsafe).}:
    return msaMain(argv, "msa")

proc main(argv: var seq[string]): int =
  return msaMain(argv, "fu-msa")

...
type
  PeRecord*     = tuple[seq1, qual1, name1, comment1, seq2, qual2, name2, comment2: string, paired: bool, status, lastChar: int]

proc readIlv*[T](r1: var Bufio[T], r: var FastxRecord): bool {.discardable.} =
  var
    sq: FastxRecord
  ...
  if (r1.readFastx(sq)):
    r.name     = sq.name
    r.seq1     = sq.seq
    r.comment1 = sq.comment
    r.qual1    = sq.qual
  if (r1.readFastx(sq)):
    r.name2    = sq.name
    r.seq2     = sq.seq
    r.comment2 = sq.comment
    r.qual2    = sq.qual

  if r.seq1.len != r.qual1.len or r.seq2.len != r.qual2.len:
    return false
  

proc readPe*[T](r1,r2: var Bufio[T], r: var FastxRecord): bool {.discardable.} =
  var
    sq: FastxRecord
